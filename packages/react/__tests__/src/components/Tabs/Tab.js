...
test('focuses the tab whose value matches its index', async () => {
  mount(<Tab value={initialValue} index={0} />);
  expect(document.activeElement.id).toEqual('tab-0');
});

test('does not focus the tab whose value does not match its index', async () => {
  mount(<Tab value={initialValue} index={1} />);
  expect(document.activeElement.outerHTML).toEqual('<body></body>');
});
