...
const ActionListItem = forwardRef<HTMLLIElement, ActionListItemProps>(
  ({
    ...
    const { role: contextRole, onAction: onActionListAction, selectionType } = useActionListContext()
    const handleAction = useMemo(() => {
      return (event: onActionEvent) => {
        ...
        if (typeof onActionListAction === 'function') {
          onActionListAction(key || labelRef?.current?.innerText.trim() || '', event)
        }
      }
    ...
    return (
      <ListboxOption
        ...
        {...props}
      >
        <span className="ActionListItem__label" ref={labelRef}>{children}</span>
        {description && (<span className="ActionListItem__description">{description}</span>)}
        {allowsSelection && <Icon type="check" />}
      </ListboxOption>
    )
  }
)
