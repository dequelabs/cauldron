...
```jsx example
<ActionList>
  <ActionListItem description="First item description">
    First item
  </ActionListItem>
  <ActionListItem description="Second item description">
    Second item
  </ActionListItem>
  <ActionListItem description="Third item description">
    Third item
  </ActionListItem>
</ActionList>
```
...
<ActionList>
  ...
  <ActionListItem leadingIcon="trash" trailingIcon="caution">
    Delete
  </ActionListItem>
</ActionList>
```
...
function SingleSelectionExample() {
  ...
  return (
    <ActionList role="menu" selectionType="single" onAction={handleAction}>
      <ActionListItem selected={selected === 'First item'}>
        First item
      </ActionListItem>
      <ActionListItem selected={selected === 'Second item'}>
        Second item
      </ActionListItem>
      <ActionListItem selected={selected === 'Third item'}>
        Third item
      </ActionListItem>
    </ActionList>
  );
...
function MultipleSelectionExample() {
  ...
  return (
    <ActionList role="menu" selectionType="multiple" onAction={handleAction}>
      <ActionListItem selected={selected.includes('First item')}>
        First item
      </ActionListItem>
      <ActionListItem selected={selected.includes('Second item')}>
        Second item
      </ActionListItem>
      <ActionListItem selected={selected.includes('Third item')}>
        Third item
      </ActionListItem>
    </ActionList>
  );
...
<Note>
  The `selectionType` prop in combination with the `role` prop is important
  depending on what selection states are needed, as this controls the underlying
  aria properties that is communicated for users who use assistive technology.
</Note>
  ...
  props={[
    {
      ...
      description:
        'Determines whether items can be selected, and if so, whether multiple selections are allowed.'
    },
    {
      name: 'onAction',
  ...
  props={[
    {
      ...
      type: 'string',
      description:
        'Unique identifier for the action item. If not provided, the trimmed text content will be used.'
    },
    {
      name: 'description',
    ...
    },
    {
      ...
      defaultValue: 'false',
      description:
        'Whether the item is currently selected. Only applicable when an ancestor ActionList or ActionListGroup has a selectionType.'
    },
    {
      name: 'variant',
      type: ['default', 'danger'],
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      type: ['single', 'multiple', 'null'],
      description:
        'Overrides the parent ActionList selectionType for this group.'
    },
    {
      name: 'onAction',
      type: '(event: React.MouseEvent | React.KeyboardEvent) => void',
