...
<Note>
  Table width by default is set to 100%. If a different max-width is desired,
  you can simply use your own CSS or `--table-width` to apply it.
</Note>
...
<Note>
  As a best practice, when a table has sorted columns an author should indicate
  which column is currently sorted by setting the sort direction for the active
  sorted column.
</Note>
...
### With Row Headings (Cell Variant) and Column Headings
...
<Table
  layout="grid"
  columns={[{ width: 'max-content', align: 'start' }, { align: 'start' }]}
>
  <TableHead>
    <TableRow>
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      description:
        'Only allowed when the table has a grid layout. Sets the column widths and alignments for each column.'
    }
  ]}
/>
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      description:
        'Only allowed when the table has a grid layout. Overrides any column alignments for this table header.'
    },
    {
      name: 'variant',
      type: ['header', 'cell'],
      description: 'Visually display the header as a table cell',
      defaultValue: 'header'
    }
  ]}
/>
  ...
  props={[
    {
      ...
      type: ['start', 'center', 'end'],
      description:
        'Only allowed when the table has a grid layout. Overrides any column alignments for this table cell.'
    }
  ]}
/>
