...
<Note>
  Be sure to always include an accessible name if rendering something visual
  that does not have any text content, like an image or an icon - or else the
  underlying button will be inaccessible to users using assistive technology
  (AT).
</Note>
...
<ol className="ExampleProgress">
  <li>
    <TooltipTabstop tooltip={`Step 1: Current`}>1</TooltipTabstop>
  </li>
  <li>
    <TooltipTabstop tooltip={`Step 2: Future`}>2</TooltipTabstop>
  </li>
  <li>
    <TooltipTabstop tooltip={`Step 3: Future`}>3</TooltipTabstop>
  </li>
</ol>
```
