...
When `disableFlip` is set, the overlay will remain at its configured `placement` and will not automatically reposition (flip) when space is limited.
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      type: 'boolean',
      defaultValue: 'false',
      description: 'When set, disables automatic flip repositioning of the overlay when it overflows its boundary.'
    },
    {
      name: 'focusTrap',
      type: 'boolean',
