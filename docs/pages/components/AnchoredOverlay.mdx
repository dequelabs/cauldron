...
When `disableFlip` is set, the overlay will not flip to an alternative placement when space is limited. The overlay will remain at its configured `placement`, though it may still shift along its axis to stay within clipping boundaries.

<Note>
  Use `disableFlip` with caution. When flipping is disabled, the overlay may render partially or fully outside the visible viewport if there is not enough space at the configured placement. This can create accessibility issues where content is announced by screen readers but not visible to sighted users. This also disables auto placement and top-overflow prevention.
</Note>
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      type: 'boolean',
      defaultValue: 'false',
      description: 'When set, disables automatic flip repositioning of the overlay when it overflows its boundary. This also disables auto placement and top-overflow prevention. The overlay may render outside the visible viewport.'
    },
    {
      name: 'focusTrap',
      type: 'boolean',
