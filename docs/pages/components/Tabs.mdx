...
function ThinTabs() {
  ...
  return (
    <>
      ...
      <TabPanel ref={tabRef1}>Panel for Thin Tabs (1)</TabPanel>
      <TabPanel ref={tabRef2}>Panel for Thin Tabs (2)</TabPanel>
      <TabPanel ref={tabRef3}>Panel for Thin Tabs (3)</TabPanel>
    </>
  );
}
```
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      type: 'string',
      description:
        'A label for tabs is required. This means you must provide either an aria-label or aria-labelledby prop.'
    },
    {
  ...
  props={[
    {
      ...
      description:
        'Pointer to the targeted TabPanel to display when the tab is active.'
    }
  ]}
/>
