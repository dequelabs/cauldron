...
The Drawer component can also be contain focus via a focus trap when the `focusTrap` property is set to true.

```jsx example
function DrawerFocusExample() {
  const [open, setOpen] = useState(false);
  const toggleOpen = () => setOpen(!open);
  return (
    <>
      <IconButton label="Toggle Focus Drawer" icon="hamburger-menu" onClick={toggleOpen} />
      <Drawer open={open} onClose={toggleOpen} position="left" focusTrap>
        <Panel style={{ height: '100vh' }}>
          <p>
            You opened me!
          </p>
          <Button variant="primary">Ok</Button>
          <Button variant="tertiary">Nevermind</Button>
        </Panel>
      </Drawer>
    </>
  )
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      type: 'boolean',
      defaultValue: 'false',
      description: 'When set to true, the drawer will act as a modal dialog displaying a Scrim behind the drawer and trapping focus within the drawer.'
    },
    {
      name: 'focusOptions',
      type: 'object',
