  ...
  return (
    <>
      ...
      <Drawer open={open} onClose={toggleOpen} position={position}>
        <Panel
          style={{
            height: ['left', 'right'].includes(position) && '100vh',
            width: ['top', 'bottom'].includes(position) && '100vw'
          }}
        >
          You opened me!
        </Panel>
  ...
  props={[
    {
      ...
      description:
        'The anchor position of the Drawer. Sets where the Drawer will appear when opened.',
      required: true
    },
    {
      name: 'open',
    ...
    },
    {
      ...
      defaultValue: 'modal',
      description:
        'When set to "modal", the drawer will act as a modal dialog displaying a backdrop behind the drawer and trapping focus within the drawer. Otherwise, "non-modal" behavior will not trap focus and will allow the user to interact with content outside of the drawer.'
    },
    {
      name: 'focusOptions',
      type: 'object',
      description:
        'Advanced customization of focus state when the Drawer open state changes.'
    },
    {
      name: 'onClose',
      type: 'function',
