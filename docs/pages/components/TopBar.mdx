...
```jsx
import { TopBar, TopBarItem, TopBarMenu } from '@deque/cauldron-react';
```
...
<Note>
  When using `MenuBar` within `TopBar`, `MenuBar` automatically sets
  `tabIndex="0"` on direct children. You _must_ set `tabindex="-1"` on any
  descendant components to prevent extra tabstops from occurring as `MenuBar`
  manages keyboard navigation automatically with <kbd>tab</kbd> and arrow key
  navigation.
</Note>
...
<Note>
  With `renderInTrigger`, the menu renders inside the `TopBarItem`. Because
  `menuitem` takes its accessible name from its content, you _must_ wrap the
  trigger's visible label in an element with the `labelId` provided to the
  trigger function. Without it, screen readers announce every menu item as part
  of the trigger's name.
</Note>
  ...
  return (
    <TopBar>
      <MenuBar>
        ...
        >
          <ActionList>
            <ActionListLinkItem href="#target-1">
              Menu Link 1
            </ActionListLinkItem>
            <ActionListLinkItem href="#target-2">
              Menu Link 2
            </ActionListLinkItem>
            <ActionListLinkItem href="#target-3">
              Menu Link 3
            </ActionListLinkItem>
          </ActionList>
        </ActionMenu>
...
<ComponentProps
  ...
  props={[
    ...
    },
    {
      ...
      defaultValue: 'true',
      description:
        'Automatically clicks links when a click occurs on TopBarItem.'
    },
    {
      name: 'menuItemRef',
