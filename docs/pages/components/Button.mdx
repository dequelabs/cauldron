---
title: Button
description: An interactive component that performs a programmable action when activated by the user.
source: https://github.com/dequelabs/cauldron/tree/develop/packages/react/src/components/Button/index.tsx
---

import { Icon, Button, Code, BadgeLabel } from '@deque/cauldron-react'

```js
import { Button } from '@deque/cauldron-react'
```

## Examples

### Primary

Primary buttons are the default variant for the `Button` component.

```jsx example
<Button>Primary</Button>
```

### Secondary

Secondary buttons are used for actions that are not the primary action.

```jsx example
<Button variant="secondary">Secondary</Button>
```

### Tertiary

Tertiary buttons are an alternate style of button with less visual prominence than primary or secondary actions.

```jsx example
<Button variant="tertiary">Tertiary</Button>
```

### Danger

Danger buttons are used to indicate destructive or irreversible actions that require careful consideration from the user.

```jsx example
<Button variant="danger">Danger</Button>
```

Alternatively, use danger-secondary when you need to indicate a destructive action with less visual prominence.

```jsx example
<Button variant="danger-secondary">Danger Secondary</Button>
```

### Button with Icon

Icons can be optionally appended to the left, or to the right of the button's text.

```jsx example
<Button variant="secondary">
  <Icon type="plus" />
  Add New User
</Button>
<Button variant="secondary">
  Add New User
  <Icon type="plus" />
</Button>
<Button variant="secondary">
  <Icon type="add-user" />
  Add New User
  <Icon type="plus" />
</Button>
```

### Link

When you need a semantic button with the visual appearance of a link.

```jsx example
<Button variant="link">Link</Button>
```

### Tag

```jsx example
<Button variant="tag">Tag</Button>
<Button variant="tag" size="small">Small Tag</Button>
```

### Badge

```jsx example
<Button variant="badge">Badge</Button>
```

### Badge with Label

```jsx example
<Button variant="badge">
  <BadgeLabel>Label:</BadgeLabel>
  value
</Button>
```

### Additional Button States

```jsx example
<Button disabled>Primary Disabled</Button>
<Button variant="secondary" thin>Secondary Thin Button</Button>
<Button variant="danger">
  <Icon type="caution" />
  Danger With Icon
</Button>
```

## Props

<ComponentProps
  children={{
    required: true
  }}
  props={[
    {
      name: 'variant',
      type: ['primary', 'secondary', 'tertiary', 'error', 'link', 'tag', 'badge', 'danger', 'danger-secondary'],
      defaultValue: 'primary',
      description: 'Visual style of the button.'
    },
    {
      name: 'thin',
      type: 'boolean',
      description: 'Thinner visual style of button.'
    },
    {
      name: 'buttonRef',
      type: 'function',
      description: 'Ref function for the button element'
    },
    {
      name: 'size',
      type: ['default', 'small'],
      defaultValue: 'default',
      description: 'Size of the button. Only applicable when variant is "tag".'
    }
  ]}
/>

## Related Components

- [IconButton](./IconButton)
- [Badge](./Badge)