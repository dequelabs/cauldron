...
Cauldron supports light and dark variants of components, with the default variant as "light". To provide variables for both light and dark modes, include light variable value in `:root` scope, with the dark variable values in a `.cauldron--theme-dark` scope:
